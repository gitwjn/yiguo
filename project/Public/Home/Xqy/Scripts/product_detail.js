...
$(function () {
    ...
    $('.detail-tab li').click(function () {
        ...
        if (i == 1) {
            ReferCommodityVote(0, $("#ProductCommentVote").find("div[_votetype='0']"));
        }
    });
    ...
    $.get("/product/GetVoteCount", { commodityId: $("#CommodityId").val() }, function (data) {
        if (data) {
    ...
    $(".comment-tab").find("span").live("click", function () {
        ...
        ReferCommodityVote(votetype, container);
    });
...
function ReferCommodityVote(votetype, container) {
    $(container).show().siblings().hide();
    if (container.html().length > 0) {
        return;
    }
    $.get("/product/CommodityVote", { CommodityId: $("#CommodityId").val(), VoteType: votetype, CurrentIndex: 0, PageSize: 10 }, function (data) {
        if (data) {
            $(container).html(data);
